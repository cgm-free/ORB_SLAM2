...
{
    //从配置文件(如TUM1.yaml)中读取设置
    cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);

    mKeyFrameSize = fSettings["Viewer.KeyFrameSize"];//TUM1.yaml中0.05
    mKeyFrameLineWidth = fSettings["Viewer.KeyFrameLineWidth"];//1
    mGraphLineWidth = fSettings["Viewer.GraphLineWidth"];//0.9
    mPointSize = fSettings["Viewer.PointSize"];//2
    mCameraSize = fSettings["Viewer.CameraSize"];//0.08
    mCameraLineWidth = fSettings["Viewer.CameraLineWidth"];//3
