    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(DetectLoop())
            {
               ...
               if(ComputeSim3())
               {
                   // Perform looLoopKeyFrameQp fusion and pose graph optimization
                   CorrectLoop();
               }
            }
        }
...
bool LoopClosing::DetectLoop()
{
    ...
    const vector<KeyFrame*> vpConnectedKeyFrames = mpCurrentKF->GetVectorCovisibleKeyFrames();// 得到与该关键帧连接的关键帧(已按权值排序)
    const DBoW2::BowVector &CurrentBowVec = mpCurrentKF->mBowVec;//mBowVec 内部实际存储的是std::map<WordId, WordValue>
    float minScore = 1;
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        // Step 5.2：将候选关键帧自己以及与自己相连的关键帧构成一个“子候选组”
        set<KeyFrame*> spCandidateGroup = pCandidateKF->GetConnectedKeyFrames();//得到与该关键帧连接（>15个共视地图点）的关键帧(没有排序的)
        // 把自己也加进去 
...
bool LoopClosing::ComputeSim3()
{
    ...
    ORBmatcher matcher(0.75,true); // 最优和次优评分的比例：0.75（默认是0.6）， 是否检查方向：true
