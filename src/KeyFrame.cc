    ...
    mpMap(pMap)
{
    ...
    mGrid.resize(mnGridCols);// 网格的列数  mnGridCols = FRAME_GRID_COLS = 64
    for(int i=0; i<mnGridCols;i++)
    {
        mGrid[i].resize(mnGridRows);// 网格的行数 mnGridRows = FRAME_GRID_ROWS = 48
        for(int j=0; j<mnGridRows; j++)
            mGrid[i][j] = F.mGrid[i][j];// mGrid 这个向量中存储的是每个图像网格内特征点的id
    }
...
void KeyFrame::ComputeBoW()
{
    ...
    {
        // 那么就从当前帧的描述子中转换得到描述子向量
        vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
        // Feature vector associate features with nodes in the 4th level (from leaves up)特征向量将特征与第4层的节点关联(从叶子层往上)
        // We assume the vocabulary tree has 6 levels, change the 4 otherwise  //?
        //将一幅图像所有的特征点（描述子向量）转化为BowVector和FeatureVector
...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    Tcw_.copyTo(Tcw);//image.copyTo(imageROI)，作用是把image的内容粘贴到imageROI；
    // cout << "Tcw:" << endl;
    // cout << Tcw << endl;
    // Tcw:                     Tcw:
    // [1, 0, 0, 0;             [0.99992007, 0.0011502529, 0.012584851, 0.11327277;
    // 0, 1, 0, 0;              -0.00052747049, 0.99878001, -0.04937695, -0.28567648;
    // 0, 0, 1, 0;              -0.012626323, 0.049366355, 0.99870098, -0.95160824;
    // 0, 0, 0, 1]               0, 0, 0, 1]
    cv::Mat Rcw = Tcw.rowRange(0,3).colRange(0,3);//rowRange(0,3)取0 1 2行，colRange(0,3)取 0 1 2列。
    // cout << "Rcw:" << endl;
    // cout << Rcw << endl; 
    // Rcw:                 Rcw:
    // [1, 0, 0;            [0.99992007, 0.0011502529, 0.012584851;
    // 0, 1, 0;             -0.00052747049, 0.99878001, -0.04937695; 
    // 0, 0, 1]             -0.012626323, 0.049366355, 0.99870098] 
    cv::Mat tcw = Tcw.rowRange(0,3).col(3);//rowRange(0,3)取0 1 2行，col(3)取第4列
    // cout << "tcw:" << endl;
    // cout << tcw << endl;
// tcw:               tcw:
// [0;               [0.11327277;
...
//  0]               -0.95160824]
    ...
    // cout << Rwc << endl;
// Rwc:               Rwc:
// [1, 0, 0;           [0.99992007, -0.00052747049, -0.012626323;    
//  0, 1, 0;            0.0011502529, 0.99878001, 0.049366355;   
//  0, 0, 1]            0.012584851, -0.04937695, 0.99870098]  
    // 和普通帧中进行的操作相同
    Ow = -Rwc*tcw;
    // cout << "Ow:" << endl;
    // cout << Ow << endl;
...
//  0]              0.93484074]
    ...
    // cout << Twc << endl;
// Twc:            Twc:
// [1, 0, 0, 0;     [0.99992007, -0.00052747049, -0.012626323, -0.12542972;       
//  0, 1, 0, 0;      0.0011502529, 0.99878001, 0.049366355, 0.33217511;       
//  0, 0, 1, 0;      0.012584851, -0.04937695, 0.99870098, 0.93484074;      
//  0, 0, 0, 1]      0, 0, 0, 1]    
    // center为相机坐标系（左目）下，立体相机中心的坐标
    // 立体相机中心点坐标与左目相机坐标之间只是在x轴上相差mHalfBaseline,
    // 因此可以看出，立体相机中两个摄像头的连线为x轴，正方向为左目相机指向右目相机 (齐次坐标)
    cv::Mat center = (cv::Mat_<float>(4,1) << mHalfBaseline, 0 , 0, 1);
    // cout << "center:" << endl; //我测试的是单目 mHalfBaseline = 0
    // cout << center << endl;
// center:     center: 
// [0;           [0; 
//  0;            0;
...
//  1]            1]
    ...
    Cw = Twc*center;//< Stereo middel point. Only for visualization
    // cout << "Cw:" << endl;
    // cout << Cw << endl;
// Cw:            Cw:
// [0;            [-0.035120446;
...
// 获取位姿 cv::Mat Tcw;    // 当前相机的位姿，世界坐标系到相机坐标系
...
// 获取(左目)相机的中心在世界坐标系下的坐标 cv::Mat Ow;     // 相机光心(左目)在世界坐标系下的坐标,这里和普通帧中的定义是一样的
...
// 获取双目相机的中心,这个只有在可视化的时候才会用到 cv::Mat Cw; ///< Stereo middel point. Only for visualization
...
{
    unique_lock<mutex> lock(mMutexPose);
    return Tcw.rowRange(0,3).colRange(0,3).clone();//为什么不用return Rcw.clone();
}
...
{
    unique_lock<mutex> lock(mMutexPose);
    return Tcw.rowRange(0,3).col(3).clone();//为什么不用return tcw.clone();
}
...
{
    {
        ...
        // 新建或更新连接权重   mConnectedKeyFrameWeights：与该关键帧连接（至少15个共视地图点）的关键帧与权重
        if(!mConnectedKeyFrameWeights.count(pKF)) //count函数返回的是一个容器中，某一元素出现的次数，对于map<key,value>，即返回key出现的次数，但是map中的key是不允许重复出现的，故count函数返回值只能是1（存在）或0（不存在）。
            //在map<KeyFrame*,int> mConnectedKeyFrameWeights容器中，找pKF
            // count函数返回0，说明mConnectedKeyFrameWeights中没有pKF，新建连接
            mConnectedKeyFrameWeights[pKF]=weight; //新增一个map数据
    ...
    UpdateBestCovisibles();//按照权重从大到小对连接（共视）的关键帧进行排序
}
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    {
        // push_front 后变成从大到小 //push_front(elem);//在容器开头插入一个元素
        lKFs.push_front(vPairs[i].second);//vPairs[i].second中，小的先入栈，大的后入栈。传到list后，就变成从大到小了。
 ...
 */
void KeyFrame::EraseMapPointMatch(const size_t &idx)//传入该MapPoint在KF中的索引
{
    unique_lock<mutex> lock(mMutexFeatures);
    // NOTE 使用这种方式表示其中的某个地图点被删除
    mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);//空指针转换为任何目标类型的指针  
    //除了用迭代器获取vector容器中元素，[ ]和at也可以，返回索引idx所指的数据
}
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mvpMapPoints;//返回关键点对应的地图点
}
...
//*************************************测试vPairs********************************************************
// void printVector(vector<pair<int,KeyFrame*> >& v) 
// {
//     for (vector<pair<int,KeyFrame*> >::iterator it = v.begin(); it != v.end(); it++) {
//     cout << "权重int： " << it->first<< " 关键帧KeyFrame： " << it->second->mnId << endl;
// } 
//     cout << endl;
// }
//*********************************************************************************************
...
void KeyFrame::UpdateConnections()
{
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)// 迭代器 与关键点keypoints相关的地图点MapPoints 
    {
        MapPoint* pMP = *vit;
        if(!pMP)
            continue;
        ...
        //map<观测到该地图点MapPoint的关键帧KF，该地图点MapPoint在关键帧KF中的索引> observations
        map<KeyFrame*,size_t> observations = pMP->GetObservations();

        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)// 迭代器   map<观测到该地图点MapPoint的关键帧KF，该地图点MapPoint在关键帧KF中的索引> 
        {
            // 除去自身，自己与自己不算共视
            if(mit->first->mnId==mnId)//mnId： Current Frame id.当前帧ID;  mit->first就是map的第一个参数KeyFrame*，mit->first->mnId 就是 KeyFrame ->mnId
                continue;
            // 这里的操作非常精彩！
            // map[key] = value，当要插入的键存在时，会覆盖键对应的原来的值。如果键不存在，则添加一组键值对。map不允许容器中有重复key值元素
            // mit->first 是地图点看到的关键帧，同一个关键帧看到的地图点会累加到该关键帧计数
            // 所以最后KFcounter  map<KeyFrame*,int> 第一个参数表示某个关键帧，第2个参数表示该关键帧看到了多少当前帧的地图点，也就是共视程度
            KFcounter[mit->first]++;// map[key] = value。即value++，关键帧看到当前帧的地图点+1
        }
    }
    ...
    vector<pair<int,KeyFrame*> > vPairs;    //pair对组
    vPairs.reserve(KFcounter.size());//如果数据量较大，可以一开始利用reserve预留空间，减少vector在动态扩展容量时的扩展次数
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        if(mit->second>=th)
        {
            ...
            vPairs.push_back(make_pair(mit->second,mit->first));//push_back(pair); //在容器尾部添加一个对组pair数据     
            // 对方关键帧也要添加这个信息
            // 更新KFcounter中该关键帧的mConnectedKeyFrameWeights
            // 更新其它KeyFrame的mConnectedKeyFrameWeights，更新其它关键帧与当前帧的连接权重
            (mit->first)->AddConnection(this,mit->second);//为当前关键帧新建或更新和其他关键帧的连接权重
        }
    }
    ...
    //********************************测试vPairs**********************************************
    // cout << "当前KeyFrame的ID号" << mnId << endl;
    // printVector(vPairs);
    // cout << endl;
// 当前KeyFrame的ID号129
// 权重int： 20 关键帧KeyFrame： 37
// 权重int： 20 关键帧KeyFrame： 36
// 权重int： 22 关键帧KeyFrame： 34
// 权重int： 27 关键帧KeyFrame： 64
// 权重int： 32 关键帧KeyFrame： 33
// 权重int： 33 关键帧KeyFrame： 65
// 权重int： 36 关键帧KeyFrame： 39
// 权重int： 42 关键帧KeyFrame： 66
// 权重int： 45 关键帧KeyFrame： 31
// 权重int： 59 关键帧KeyFrame： 67
// 权重int： 99 关键帧KeyFrame： 96
// 权重int： 110 关键帧KeyFrame： 97
// 权重int： 114 关键帧KeyFrame： 117
// 权重int： 130 关键帧KeyFrame： 0
// 权重int： 132 关键帧KeyFrame： 50
// 权重int： 142 关键帧KeyFrame： 52
// 权重int： 142 关键帧KeyFrame： 53
// 权重int： 147 关键帧KeyFrame： 115
// 权重int： 149 关键帧KeyFrame： 19
// 权重int： 152 关键帧KeyFrame： 107
// 权重int： 157 关键帧KeyFrame： 21
// 权重int： 164 关键帧KeyFrame： 1
// 权重int： 165 关键帧KeyFrame： 92
// 权重int： 173 关键帧KeyFrame： 109
// 权重int： 179 关键帧KeyFrame： 47
// 权重int： 204 关键帧KeyFrame： 106
// 权重int： 220 关键帧KeyFrame： 110
// 权重int： 239 关键帧KeyFrame： 86
// 权重int： 276 关键帧KeyFrame： 123
// 权重int： 323 关键帧KeyFrame： 127
// 权重int： 326 关键帧KeyFrame： 128
    //****************************************************************************************
    // 将排序后的结果分别组织成为两种数据类型
    list<KeyFrame*> lKFs;
    list<int> lWs;
    ...
    {
        // push_front 后变成了从大到小顺序  push_front(elem);//在容器开头插入一个元素
...

    {
        ...
        if(mbFirstConnection && mnId!=0)
        {
            ...
            mpParent = mvpOrderedConnectedKeyFrames.front();//front(); //返回第一个元素
