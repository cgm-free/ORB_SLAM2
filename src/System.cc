        			 ...
        			 mbDeactivateLocalizationMode(false)		//没有这个模式转换标志
{
    ...
    //Check settings file
    // 格式 FileStorage(const String& filename, int flags, const String& encoding=String());
    ...
    //建立一个新的空的ORB字典
    //ORBVocabulary.h 增加了 ORBVocabulary 这个自定义数据类型 对象
        //     namespace ORB_SLAM2
        // {
    ...
    mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);//*mpVocabulary解引用  KeyFrameDatabase::KeyFrameDatabase (const ORBVocabulary &voc):
    ...
    // mpFrameDrawer 固定画布大小为640*480，白色
    mpFrameDrawer = new FrameDrawer(mpMap);//  System.h中的Map* mpMap;FrameDrawer* mpFrameDrawer;  FrameDrawer.cc的构造函数FrameDrawer::FrameDrawer(Map* pMap):mpMap(pMap)
    //FrameDrawer* mpFrameDrawer = new FrameDrawer(Map* pMap):mpMap(pMap);
    //从配置文件(如TUM1.yaml)中读取设置 主要设置关键帧框的大小和线宽，graph的线宽，点Point的大小，相机框的大小和线宽
    mpMapDrawer = new MapDrawer(mpMap, strSettingsFile);

    //在本主进程中初始化追踪线程
    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    mpTracker = new Tracking(this,						//现在还不是很明白为什么这里还需要一个this指针  TODO  System *pSys, //系统实例
    						 mpVocabulary,				//字典
    						 mpFrameDrawer, 			//帧绘制器
    						 mpMapDrawer,				//地图绘制器
                             mpMap, 					//地图
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    //获取相机位姿的估计结果
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,timestamp); //跟踪器指针mpTracker。它接收帧并计算相关的相机姿态。

    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;//mvpMapPoints 每个特征点对应的MapPoint
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;//校正  原始左图像提取出的特征点  的
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    //自定义排序: 需用户指定排序规则KeyFrame::lId,将 [vpKFs.begin(),vpKFs.end())区间内的元素按照用户指定的顺序排列。
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);

    // Transform all keyframes so that the first keyframe is at the origin.
    // After a loop closure the first keyframe might not be at the origin.
    // 到原点的转换，获取这个转换矩阵   获取位姿的逆 Twc.clone()
