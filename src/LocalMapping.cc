...
// void printList(const list<MapPoint*> & L) 
// {
//     for (list<MapPoint*>::const_iterator it = L.begin(); it != L.end(); it++) 
//     {
//         cout << *it << " ";//0x7f0d1ea4b140 0x7f0d1f0053a0 0x7f0d1d7dfb50 0x7f0d1ec71730 0x7f0d1ea4aa70 0x7f0d1ea49df0 0x7f0d1ee709d0
//     } 
//         cout << "mlpRecentAddedMapPoints = " << endl;
// }
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    const vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();// 返回关键点对应的地图点，存储在vector容器对象vpMapPointMatches中
    // 对当前处理的这个关键帧中的所有的地图点展开遍历
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        MapPoint* pMP = vpMapPointMatches[i];
        if(pMP)//不是空指针时执行；如果特征点（关键点）没有对应的地图点,那么将存储一个空指针
        {
            if(!pMP->isBad())// 没有经过 MapPointCulling 检测的MapPoints, 认为是坏掉的点
            {
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame))//检查该地图点是否在关键帧中（有对应的二维特征点）
                {
                ...
                else // this can only happen for new stereo points inserted by the Tracking
                {
                    ...
                    // printList(mlpRecentAddedMapPoints);
                    // cout <<"mlpRecentAddedMapPoints第一个元素：" << mlpRecentAddedMapPoints.front() <<endl;//存的指针 0x7fa341a40780
                }
            }
        }
    }    
...
void LocalMapping::MapPointCulling()
{
	...
	// Step 2：遍历检查新添加的地图点
    while(lit!=mlpRecentAddedMapPoints.end())//lit是地图点的迭代器
    {
        MapPoint* pMP = *lit;
        if(pMP->isBad())
        {
        ...
        {
            // 当前关键帧ID-创建该地图点pMP的关键帧ID 大于等于 2
            ...
            lit++;//由于链表的存储方式并不是连续的内存空间，因此链表list中的迭代器只支持前移和后移，属于双向迭代器
    }
}
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        {
            // Step 6.1：取出匹配特征点
            // vMatchedIndices = vnMatches12 保存参考帧F1和F2匹配关系，index保存是F1对应特征点索引，值保存的是匹配好的F2特征点索引
            // 当前匹配对在当前关键帧中的索引
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        ...
        // 1.如果地图点能匹配关键帧的特征点，并且该特征点有对应的地图点，那么选择观测数目多的替换两个地图点
        // 2.如果地图点能匹配关键帧的特征点，并且该特征点没有对应的地图点，那么为该点添加该投影地图点
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    {  
                         //当前关键帧 的 某一个共视关键帧 的 金字塔尺度：scaleLevel
                        const int &scaleLevel = pKF->mvKeysUn[i].octave;//观测到的关键点在金字塔的第几层  
                        ...
                        for(map<KeyFrame*, size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            ...
                            //所有能观测到这个地图点（当前关键帧 的 某一个共视关键帧对应的所有地图点的某个地图点 ）的关键帧 中的某个关键帧 的 金字塔尺度：scaleLeveli
                            const int &scaleLeveli = pKFi->mvKeysUn[mit->second].octave;//观测到的关键点在金字塔的第几层  
