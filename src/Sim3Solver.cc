...
void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2)
{
    ...
    // 然后我们N进行特征值分解，求得最大特征值对应的特征向量就是待求的用四元数表示的旋转

    cv::Mat eval, evec;  // val vec   特征值    特征向量
    // 特征值默认是从大到小排列，所以evec.row(0) 是最大值
    cv::eigen(N,eval,evec); 
    cout << "特征值eval = " << eval << endl;      
    cout << "特征向量evec = " << evec << endl;  
    cout << "最大特征值对应的特征向量evec[0]就是要求的四元数(q0 q1 q2 q3) = " << evec.row(0) << endl;  
    ...
    cout << "将(q1 q2 q3)放入(四元数的虚部)vec= " << vec << endl;      
    ...
    cout << "旋转的角度 = atan2(四元数的虚部/实部) = " << ang << endl;      
     
    // vec/norm(vec)归一化得到归一化后的旋转向量,然后乘上角度得到包含了旋转轴和旋转角信息的旋转向量vec
    vec = 2*ang*vec/norm(vec); //Angle-axis x. quaternion angle is the half
    cout << "vec/norm(vec)归一化得到归一化后的旋转向量,然后乘上角度得到包含了旋转轴和旋转角信息的旋转向量 vec = " << vec << endl;      

    mR12i.create(3,3,P1.type());
    // 旋转向量（轴角）转换为旋转矩阵
    cv::Rodrigues(vec,mR12i); // computes the rotation matrix from angle-axis
    cout << "旋转向量(轴角)vec 转换为 旋转矩阵mR12i = " << mR12i << endl;      

    // Step 5: Rotate set 2
    // 利用刚计算出来的旋转将三维点旋转到同一个坐标系，P3对应论文里的 r_l,i', Pr1 对应论文里的r_r,i'
    cv::Mat P3 = mR12i*Pr2;
    cout << "利用刚计算出来的旋转将三维点旋转到同一个坐标系 P3 = mR12i*Pr2 = " << P3 << endl;      

    // Step 6: 计算尺度因子 Scale
    if(!mbFixScale)// 单目的时候需要计算尺度
    {
        ...
        double num = Pr1.dot(P3);// A.dot(B)操作相当于数学向量运算中的点乘，也叫向量的内积、数量积。
        cout << "Pr1 = " << Pr1 << endl;
        cout << "P3 = " << P3 << endl;      
        cout << "计算尺度因子的分子numerator = Pr1.dot(P3) = " << num << endl;      

        // 准备计算分母
        cv::Mat aux_P3(P3.size(),P3.type());
        aux_P3=P3;
        // 先得到平方
        cv::pow(P3,2,aux_P3);//  将P3每个元素平方 =  aux_P3 
        cout << "aux_P3 = " << aux_P3 << endl;      
        ...
        cout << "计算尺度因子的分母denominator = " << den << endl;      

        ms12i = num/den;
        cout << "尺度因子s12i = num/den  = " << den << endl;      

    }
    else // 双目和RGBD的时候就不需要计算尺度
        ms12i = 1.0f;
    ...
    cout << "平移的最优解t*   mt12i = O1 - ms12i*mR12i*O2  = " << mt12i << endl;      
    
    // Step 8: 计算双向变换矩阵，目的是在后面的检查的过程中能够进行双向的投影操作

    // Step 8.1 用尺度，旋转，平移构建变换矩阵T12
    mT12i = cv::Mat::eye(4,4,P1.type());

    cv::Mat sR = ms12i*mR12i;
    cout << "计算变换矩阵T12的sR = ms12i*mR12i  = " << sR << endl;      

    //         |sR t|
    // mT12i = | 0 1|
    sR.copyTo(mT12i.rowRange(0,3).colRange(0,3));
    mt12i.copyTo(mT12i.rowRange(0,3).col(3));
    cout << "计算变换矩阵T12的t   = " << mt12i << endl;      
    cout << "计算变换矩阵T12   = " << mT12i << endl;      
    ...
    cout << "计算逆变换矩阵T21的 1/s *R^T  = " << sRinv << endl;      

    sRinv.copyTo(mT21i.rowRange(0,3).colRange(0,3));
    cv::Mat tinv = -sRinv*mt12i;
    cout << "计算逆变换矩阵T21的 -1/s *R^T *t12  = " << tinv << endl;      
    tinv.copyTo(mT21i.rowRange(0,3).col(3));
    cout << "计算逆变换矩阵T21  = " << mT21i << endl;      
