        ...
        mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
    ...
    // Step 1 从配置文件中加载相机参数fx fy cx cy
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im,const double &timestamp)
{
    ...
    // Step 2 ：构造Frame
    //调用单目帧构造函数  Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    //判断该帧是不是初始化
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) //没有成功初始化的前一个状态就是NO_IMAGES_YET
        mCurrentFrame = Frame(
            mImGray,                //灰度图
            timestamp,              //时间戳
            mpIniORBextractor,      //初始化ORB特征点提取器会提取2倍的指定特征点数目  //ORB特征点提取器的句柄
            mpORBVocabulary,        //ORB字典的句柄
            mK,                     //相机的内参数矩阵
            mDistCoef,              //相机的去畸变参数
            mbf,                    //baseline*f
            mThDepth);              //区分远近点的深度阈值
