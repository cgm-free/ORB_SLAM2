...
namespace DBoW2
{
   ...
   * @tparam TDescriptor
   * @tparam F
   * @param[in] features      图像中所有的特征点  （描述子向量vCurrentDesc）
      ...
      BowVector &v, FeatureVector &fv, int levelsup) const
  {
    ...
    if (m_weighting == TF || m_weighting == TF_IDF)
    {
      ...
      for (fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
      {
        ...
        NodeId nid;  // FeatureVector 里的NodeId，用于加速搜索；距离叶子节点深度为level up对应的节点的id。
        WordValue w; // 叶子节点Word对应的权重

        //  将当前描述子转化为Word id， Word weight，节点所属的父节点id（这里的父节点不是叶子的上一层，它距离叶子深度为levelsup）
        // 所有权重均可表示为节点权重NodeWeights和在线权重的乘积，其中节点权重为构建字典时确定，在线权重在输入每张图像时计算
